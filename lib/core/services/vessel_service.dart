...
class VesselService {
  ...
  Future<List<Map<String, dynamic>>?> getDataKapalGeosat() async {
    ...
    final cachedData = prefs.getString('cachedDataKapalGeosat');
    ...
    try {
      ...
      // Debug print to indicate fetching new data from the database
    ...
    ''');
      ...
      if (results.isNotEmpty) {
        for (var row in results) {
          ...
          switch (timeZonePreferences) {
            case 'UTC':
              adjustedTimestamp =
                  originalTimestamp.subtract(Duration(hours: 7));
              adjustedBroadcast =
                  originalBroadcast.subtract(Duration(hours: 7));
              break;
            case 'UTC+7':
              adjustedTimestamp = originalTimestamp;
              adjustedBroadcast = originalBroadcast;
      ...
      prefs.setString('cachedDataKapalGeosat', jsonEncode(dataKapalGeosatList));
